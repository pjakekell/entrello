...
export function SaveMediaButton({ loading }: SaveMediaButtonProps) {
  return (
    <button
      type={loading ? "button" : "submit"}
      className={`${
        loading ? "bg-gray-400 cursor-not-allowed disabled" : "bg-brand-500"
    ...
    >
      {loading ? (
        ...
        <UploadIcon aria-hidden="true" className="text-white h-5 w-5 mr-2" />
      )}
      Save Media
    </button>
  );
