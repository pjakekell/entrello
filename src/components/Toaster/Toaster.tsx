import React from "react";
/* This example requires Tailwind CSS v2.0+ */
import { Fragment } from "react";
import { Transition } from "@headlessui/react";
import { CheckCircleIcon, ExclamationIcon } from "@heroicons/react/outline";
import { XIcon } from "@heroicons/react/solid";
import { useSelector, useDispatch } from "react-redux";
import { selectMsg, setMsg, IMsg } from "./logic";
import { classNames } from "../../utils/misc";

const levelStates: any = {
  info: () => (
    <ExclamationIcon className="h-6 w-6 text-blue-600" aria-hidden="true" />
  ),
  warn: () => (
    <ExclamationIcon className="h-6 w-6 text-yellow-500" aria-hidden="true" />
  ),
  error: () => (
    <ExclamationIcon className="h-6 w-6 text-red-500" aria-hidden="true" />
  ),
  success: () => (
    <CheckCircleIcon className="h-6 w-6 text-green-400" aria-hidden="true" />
  ),
};

const levelFGColor: any = {
  info: () => ({
    title: "text-gray-600",
    desc: "text-gray-500",
  }),
  warn: () => ({
    title: "text-yellow-800",
    desc: "text-yellow-700",
  }),
  error: () => ({
    title: "text-red-700",
    desc: "text-red-600",
  }),
  success: () => ({
    title: "text-green-700",
    desc: "text-green-600",
  }),
};

export default function Toaster() {
  const msg = useSelector(selectMsg) as IMsg;
  const dispatch = useDispatch();
  const show = msg && msg.title ? true : false;

  const handleClearMsg = () => {
    dispatch(setMsg(null));
  };

  const col =
    msg && msg.level ? levelFGColor[msg.level]() : levelFGColor.info();

  return (
    <>
      {/* Global notification live region, render this permanently at the end of the document */}
      <div
        aria-live="assertive"
        className="fixed z-100 inset-0 flex items-end px-4 py-6 pointer-events-none sm:p-6 sm:items-start"
      >
        <div className="w-full flex flex-col items-center space-y-4 sm:items-end">
          {/* Notification panel, dynamically insert this into the live region when it needs to be displayed */}
          <Transition
            show={show}
            as={Fragment}
            enter="transform ease-out duration-300 transition"
            enterFrom="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-x-2"
            enterTo="translate-y-0 opacity-100 sm:translate-x-0"
            leave="transition ease-in duration-100"
            leaveFrom="opacity-100"
            leaveTo="opacity-0"
          >
            <div className="max-w-sm w-full bg-white shadow-lg rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 overflow-hidden">
              <div className="p-4">
                <div className="flex items-start">
                  <div className="flex-shrink-0">
                    {msg && msg.level && levelStates[msg.level]
                      ? levelStates[msg.level]()
                      : levelStates["info"]()}
                  </div>
                  <div className="ml-3 w-0 flex-1">
                    <p className={classNames("text-sm font-medium", col.title)}>
                      {msg && msg.title}
                    </p>
                    {msg && msg.desc ? (
                      <p className={classNames("mt-1 text-sm", col.desc)}>
                        {msg.desc}
                      </p>
                    ) : null}
                  </div>
                  <div className="ml-4 flex-shrink-0 flex">
                    <button
                      className="bg-white rounded-md inline-flex text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                      onClick={handleClearMsg}
                    >
                      <span className="sr-only">Close</span>
                      <XIcon className="h-5 w-5" aria-hidden="true" />
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </Transition>
        </div>
      </div>
    </>
  );
}
